...
from analytics import order_book_imbalance, realized_volatility, vwap
...
class OrderBookApp:
    ...
    def _build_layout(self) -> None:
        ...
        # Quant analytics (analytics.py): rolling VWAP / realized vol / imbalance.
        self._build_stat(top, "VWAP", "vwap", TEXT)
        self._build_stat(top, "VOL", "vol", ACCENT)
        self._build_stat(top, "OBI", "obi", MUTED)
    ...
    def _update_stats(self) -> None:
        ...
        # Live quant analytics over the rolling tape window + current book.
        trades = [t for _, t in self.tape]
        v = vwap(trades)
        rv = realized_volatility(trades)
        obi = order_book_imbalance(self.book)
        self.stat_vars["vwap"].set(f"{v:.2f}" if v is not None else "—")
        self.stat_vars["vol"].set(f"{rv * 1e4:.1f}" if rv is not None else "—")  # in bps
        self.stat_vars["obi"].set(f"{obi:+.2f}" if obi is not None else "—")
